...
def get_user(request: WSGIRequest):
    ...
    return Response(
        UserSerializer(User.objects.filter(id=request.user.id).first(), many=False).data
    )
...
def get_summaries(request: WSGIRequest):
    ...
    return Response(
        SummarySerializer(
            Summary.objects.filter(user_id=request.user.id).all(), many=True
        ).data
    )
    ...
    return Response(
        SummarySerializer(
            Summary.objects.filter(
                user_id=User.objects.filter(email=user).first().id
            ).all(),
            many=True,
        ).data
    )
...
def get_city(request: WSGIRequest, city: str):
    return Response({"city": city, "name": get_city_name(city)})
